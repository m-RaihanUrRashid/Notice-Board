...
<body>
    ...
    <?php
    echo "<h1>Apply as SOD!</h1>";
    ?>
    <div id="annBox">
        <?php

        $sql = "SELECT * FROM classroom WHERE SODid IS NULL";
        $clist = mysqli_query($conn, $sql);
        if (mysqli_num_rows($clist) == 0) {
            echo "<div class=announ style=\"text-align: center;\"> No Classrooms to apply to yet. </div><br>";
        } else {
